...
let flush =
foreign "g_io_channel_flush" (ptr t_typ  @-> ptr_opt (ptr Error.t_typ) @-> returning (Core.iostatus))
let get_buffer_condition =
foreign "g_io_channel_get_buffer_condition" (ptr t_typ @-> returning (Core.iocondition_list))
let get_buffer_size =
...
let get_flags =
foreign "g_io_channel_get_flags" (ptr t_typ @-> returning (Core.ioflags_list))
let get_line_term =
...
let set_encoding =
foreign "g_io_channel_set_encoding" (ptr t_typ @-> string_opt  @-> ptr_opt (ptr Error.t_typ) @-> returning (Core.iostatus))
let set_flags =
foreign "g_io_channel_set_flags" (ptr t_typ @-> Core.ioflags_list  @-> ptr_opt (ptr Error.t_typ) @-> returning (Core.iostatus))
let set_line_term =
foreign "g_io_channel_set_line_term" (ptr t_typ @-> string_opt @-> int32_t @-> returning (void))
let shutdown =
foreign "g_io_channel_shutdown" (ptr t_typ @-> bool  @-> ptr_opt (ptr Error.t_typ) @-> returning (Core.iostatus))
let unix_get_fd =
foreign "g_io_channel_unix_get_fd" (ptr t_typ @-> returning (int32_t))
let unref =
foreign "g_io_channel_unref" (ptr t_typ @-> returning (void))
let write =
foreign "g_io_channel_write" (ptr t_typ @-> string @-> uint64_t @-> ptr uint64_t @-> returning (Core.ioerror))
