...
let f_config = field t_typ "config" (ptr ScannerConfig.t_typ)
let f_token = field t_typ "token" (Core.tokentype)
...
let f_next_token = field t_typ "next_token" (Core.tokentype)
(* TODO Struct field Scanner : union tag not implemented . *)
...
let cur_token =
foreign "g_scanner_cur_token" (ptr t_typ @-> returning (Core.tokentype))
let destroy =
foreign "g_scanner_destroy" (ptr t_typ @-> returning (void))
let eof =
foreign "g_scanner_eof" (ptr t_typ @-> returning (bool))
let get_next_token =
foreign "g_scanner_get_next_token" (ptr t_typ @-> returning (Core.tokentype))
let input_file =
...
let peek_next_token =
foreign "g_scanner_peek_next_token" (ptr t_typ @-> returning (Core.tokentype))
...
let unexp_token =
foreign "g_scanner_unexp_token" (ptr t_typ @-> Core.tokentype @-> string @-> string @-> string @-> string @-> int32_t @-> returning (void))
