...
let f_callback_funcs = field t_typ "callback_funcs" (ptr SourceCallbackFuncs.t_typ)
let f_source_funcs = field t_typ "source_funcs" (ptr SourceFuncs.t_typ)
let f_ref_count = field t_typ "ref_count" (uint32_t)
let f_context = field t_typ "context" (ptr MainContext.t_typ)
...
let f_priv = field t_typ "priv" (ptr SourcePrivate.t_typ)
let _new =
foreign "g_source_new" (ptr t_typ @-> ptr SourceFuncs.t_typ @-> uint32_t @-> returning (ptr t_typ))
let add_child_source =
foreign "g_source_add_child_source" (ptr t_typ @-> ptr t_typ @-> returning (void))
let add_poll =
foreign "g_source_add_poll" (ptr t_typ @-> ptr PollFD.t_typ @-> returning (void))
let add_unix_fd =
foreign "g_source_add_unix_fd" (ptr t_typ @-> int32_t @-> Core.iocondition_list @-> returning (ptr void))
let attach =
foreign "g_source_attach" (ptr t_typ @-> ptr_opt MainContext.t_typ @-> returning (uint32_t))
let destroy =
foreign "g_source_destroy" (ptr t_typ @-> returning (void))
let get_can_recurse =
foreign "g_source_get_can_recurse" (ptr t_typ @-> returning (bool))
let get_context =
foreign "g_source_get_context" (ptr t_typ @-> returning (ptr_opt MainContext.t_typ))
let get_current_time =
foreign "g_source_get_current_time" (ptr t_typ @-> ptr TimeVal.t_typ @-> returning (void))
let get_id =
...
let remove_poll =
foreign "g_source_remove_poll" (ptr t_typ @-> ptr PollFD.t_typ @-> returning (void))
let remove_unix_fd =
...
foreign "g_source_set_callback_indirect" (ptr t_typ @-> ptr_opt void @-> ptr SourceCallbackFuncs.t_typ @-> returning (void))
let set_can_recurse =
foreign "g_source_set_can_recurse" (ptr t_typ @-> bool @-> returning (void))
let set_funcs =
foreign "g_source_set_funcs" (ptr t_typ @-> ptr SourceFuncs.t_typ @-> returning (void))
let set_name =
...
foreign "g_source_remove_by_funcs_user_data" (ptr t_typ @-> ptr SourceFuncs.t_typ @-> ptr_opt void @-> returning (bool))
