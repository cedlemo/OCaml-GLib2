...
let new_dmy =
foreign "g_date_new_dmy" (ptr t_typ @-> uint8_t @-> Core.datemonth @-> uint16_t @-> returning (ptr t_typ))
let new_julian =
...
let set_time_val =
foreign "g_date_set_time_val" (ptr t_typ @-> ptr TimeVal.t_typ @-> returning (void))
let set_year =
