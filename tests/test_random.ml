...
let test_random_double test_ctxt =
  ...
  assert_equal ~printer true (rdouble >= 0.0);
  assert_equal ~printer true (rdouble <= 1.0)

let test_random_double_range test_ctxt =
  let start = 5.0 in
  let stop = 6.0 in
  let rdouble = GLib.Core.random_double_range start stop in
  assert_equal ~printer true (rdouble >= start);
  assert_equal ~printer true (rdouble <= stop)

let test_random_int test_ctxt =
  let open Unsigned in
  let rint = GLib.Core.random_int () in
  assert_equal ~printer true UInt32.((compare rint zero) >= 0);
  assert_equal ~printer true UInt32.((compare rint GLib.Core.c_MAXUINT32) <= 0)
...
let test_random_int_range test_ctxt =
  ...
  assert_equal ~printer true Int32.((compare rint start) >= 0);
  assert_equal ~printer true Int32.((compare rint stop) <= 0)
